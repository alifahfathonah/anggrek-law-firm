...
<?php
  ...
  echo "<script>
	...
	$save=mysqli_query($con, "INSERT INTO kegiatan 
...
<?php
    ...
    if(isset($_GET['id'])){
      $lihat = mysqli_fetch_assoc(mysqli_query($con, "SELECT * FROM kegiatan where idkegiatan='$_GET[id]'"));

      $del = mysqli_query($con, "DELETE FROM kegiatan where idkegiatan='$_GET[id]'");
      if($del){
    	  echo "<script>
                 alert('Data Berhasil Dihapus');
    					   window.location='index.php?module=kegiatan';
    				  </script>";
      }else{
        echo "<script>
...
<section class="content">
      <div class="row">
        <div class="col-md-12">
          <div class="box box-info">
            <div class="box-header with-border">
              <a href="?module=kegiatan&aksi=tambahkegiatan" class="btn btn-flat btn-primary">Tambah Kegiatan </a>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
          ...
          <th rowspan="2">Keterangan</th>
                ...
                <tbody>
				<?php
					$be = mysqli_query($con, "SELECT * FROM kegiatan WHERE nama='$_SESSION[id_client]' order by idkegiatan desc");
					$no=1;
		  ...
		  ?>
					<td><a onclick="return confirm('Maaf, Data Tidak Bisa di Edit');" href="index.php?module=kegiatan" class="btn btn-info">Edit</a>
					<?php
					}else{
					?>
					<td><a href="?module=kegiatan&aksi=editkegiatan&id=<?= $r['idkegiatan'];?>" class="btn btn-info">Edit</a>
					<?php
					}
					?>
						<a onclick="return confirm('Apakah Anda Yakin Ingin Menghapus Data Ini ?');" href="?module=kegiatan&aksi=hapuskegiatan&id=<?= $r['idkegiatan'];?>" class="btn btn-info">Hapus</a>
					</td>
				</tr>
					<?php $no++; } ?>
				</tfoot>
